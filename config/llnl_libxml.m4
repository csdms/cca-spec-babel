...
[LIBXML_REQUIRED_VERSION=2.4.0
 ...
 AC_ARG_WITH([libxml2],
	[--with-libxml2=DIR
		use libxml2 in DIR (default is to use it if found).
		e.g. --with-libxml2=/usr if xml2-config is in /usr/bin.
	]
	, ,[withval=maybe])
...
if test "$withval" = "no"; then
   ...
   if test "$vers" -ge VERSION_TO_NUMBER([echo $LIBXML_REQUIRED_VERSION]); then
	...
	if $XML2_CONFIG --libtool-libs | grep "^Usage:" 2>&1 > /dev/null; then
	    ...
	    LIBXML2_LIB_NOLIBTOOL="`$XML2_CONFIG --libs`"
	else
	...
	AC_SUBST(LIBXML2_LIB_NOLIBTOOL)
